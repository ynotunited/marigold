                ...
                <div class="checkout-layout">
                    ...
                    <div class="checkout-main">
                        ...
                        <div class="cstep">
                            ...
                            <label class="pay-opt" :class="{ on: accountChoice === 'guest' }">
                                <div class="po-row">
                                    <div class="po-label">
                                        <input type="radio" value="guest" name="account_choice" x-model="accountChoice">
                                        <span>Continue as guest</span>
                                    </div>
                                    <span class="po-ico">
                                        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="5"/><path d="M20 21a8 8 0 1 0-16 0"/></svg>
                                    </span>
                                </div>
                                <div class="collapse-box" :class="{ open: accountChoice === 'guest' }">
                                    <div>
                                        <p class="po-body">No account needed — we'll email your order confirmation and status updates.</p>
                                    </div>
                                </div>
                            </label>

                            <label class="pay-opt" :class="{ on: accountChoice === 'account' }">
                                <div class="po-row">
                                    <div class="po-label">
                                        <input type="radio" value="account" name="account_choice" x-model="accountChoice">
                                ...
                                <div class="collapse-box" :class="{ open: accountChoice === 'account' }">
                                    <div>
                                        <div style="margin-top: 18px; padding-top: 18px; border-top: 1px solid var(--line);">
                                            <p class="po-body" style="margin-bottom: 16px;">Create an account to track your orders, save your details and check out faster next time.</p>
                                            <div class="form-grid">
                                                <div class="form-row full">
                                                    <label>Password <em>*</em></label>
                                                    <input type="password" name="password" x-model="password" placeholder="Minimum 8 characters" class="field" autocomplete="new-password">
                                                </div>
                                                <div class="form-row full">
                                                    <label>Confirm Password <em>*</em></label>
                                                    <input type="password" name="password_confirm" x-model="passwordConfirm" placeholder="Repeat password" class="field" autocomplete="new-password">
                                                </div>
                                            </div>
                                            <p x-show="passwordError" class="form-error" x-text="passwordError" style="margin-top: 10px;"></p>
                                        </div>
                                    </div>
                                </div>
                            </label>
...
function checkoutPage() {
    return {
        ...
        fmtSym() {
            var c = (window.MS_CURRENCY || {}).selected || 'NGN';
            var sym = { NGN: '\u20A6', USD: '$', GBP: '\u00A3', EUR: '\u20AC', GHS: 'GH\u20B5', ZAR: 'R', KES: 'KSh', XAF: 'FCFA' };
            return (sym[c] || c + ' ');
        },
        uuidV4() {
        ...
        validateAccount() {
            this.passwordError = '';
            if (this.accountChoice !== 'account') return true;
        ...
        processPayment() {
            ...
            const payload = {
                ...
                create_account: this.accountChoice === 'account',
                password: this.accountChoice === 'account' ? this.password : '',
                items: this.selected().map(p => ({ product_id: p.id, quantity: this.qty[p.id] })),
                currency: (window.MS_CURRENCY || {}).selected || 'NGN'
            };
            if (this.deliveryMethod === 'delivery' && this.selectedCourier()) {
