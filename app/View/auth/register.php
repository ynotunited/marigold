...
<div class="card p-8 sm:p-10" x-data="registerForm()">
    ...
    <form action="<?= app_url('/register') ?>" method="POST" @submit="submit">
        <?= \App\Core\CSRF::field() ?>
    ...
    <div class="mt-8 text-center text-sm text-[var(--text-secondary)]">
        ...
        <a href="<?= app_url('/login') ?>" class="font-medium text-[var(--gold)] hover:text-white transition-colors">Sign in here</a>
    </div>
</div>
