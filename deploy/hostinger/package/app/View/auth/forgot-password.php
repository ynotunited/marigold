...
<div class="card p-8 sm:p-10" x-data="forgotForm()">
    ...
    <form action="<?= app_url('/forgot-password') ?>" method="POST" @submit="submit">
        <?= \App\Core\CSRF::field() ?>
    ...
    <div class="mt-8 text-center text-sm text-[var(--text-secondary)]">
        <a href="<?= app_url('/login') ?>" class="inline-flex items-center font-medium text-[var(--gold)] hover:text-white transition-colors">
