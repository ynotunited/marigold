...
<div class="flex items-center justify-between mb-6">
    ...
    <div class="flex items-center gap-3">
        <a href="<?= app_url('/admin/blog/create') ?>" class="btn btn-primary h-9 px-4 text-sm flex items-center gap-2">
...
<div class="bg-[#111] border border-[var(--border)] rounded-[14px] p-4 mb-6 flex items-center gap-3 overflow-x-auto">
    <div class="relative flex-grow min-w-[200px] max-w-sm shrink-0"><i data-lucide="search" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--text-muted)]"></i><input type="text" placeholder="Search titles, authors…" class="input-field w-full pl-10 h-9 text-sm bg-[var(--surface)]"></div>
    <select class="input-field h-9 text-sm bg-[var(--surface)] pr-8 min-w-[140px] shrink-0"><option value="">Category</option><option>Gift Guides</option><option>Marketing</option><option>Sustainability</option></select>
    <select class="input-field h-9 text-sm bg-[var(--surface)] pr-8 min-w-[140px] shrink-0"><option value="">Status</option><option>Published</option><option>Draft</option><option>Scheduled</option></select>
</div>
                ...
                <tr class="hover:bg-[var(--surface)]/40 transition-colors group cursor-pointer" onclick="window.location='/admin/blog/<?= $p['id'] ?>/edit'">
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center gap-2">
                            ...
                            <a href="<?= app_url('/admin/blog/') ?><?= $p['id'] ?>/edit" class="font-medium text-sm group-hover:text-[var(--gold)] transition-colors"><?= htmlspecialchars($p['title']) ?></a>
                        </div>
                    </td>
                    ...
                    <td class="px-5 py-4" onclick="event.stopPropagation()">
                        <div class="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <a href="<?= app_url('/admin/blog/') ?><?= $p['id'] ?>/edit" class="w-8 h-8 rounded-[6px] bg-[var(--surface)] border border-[var(--border)] flex items-center justify-center text-[var(--text-secondary)] hover:text-[var(--gold)] hover:border-[var(--gold)]/50 transition-colors"><i data-lucide="pencil" class="w-3.5 h-3.5"></i></a>
