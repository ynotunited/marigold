...
<div style="background: var(--ivory); color: var(--ink);" x-data="cartPage()">
    ...
    <section class="page-hero">
        <div class="container">
            <div class="crumbs"><a href="<?= app_url('/') ?>">Home</a><span>/</span><span>Cart</span></div>
            ...
            <template x-if="items.length === 0">
                <div class="cart-panel reveal" style="padding: 30px;">
                    <div class="success-state">
                        ...
                        <a href="<?= app_url('/shop') ?>" class="btn btn-gold btn-lg">Start Shopping</a>
                    </div>
                </div>
            </template>
                ...
                <div class="cart-layout">
                        ...
                        <div class="cart-panel reveal">
                            ...
                            <template x-for="(item, index) in items" :key="item.id">
                                <div class="cart-item">
                                    ...
                                    <div class="ci-price" x-text="fmt(item.price)"></div>
                                    <div class="ci-qty">
                                        <div class="qty">
                                    ...
                                    <div class="ci-total">
                                        <span class="amt" x-text="fmt(item.price * item.qty)"></span>
                    ...
                    <aside class="summary-card reveal d1">
                        ...
                        <div class="row" x-show="discount > 0" style="display: none;">
                            ...
                            <span style="color: #2e7d32; font-weight: 700;" x-text="'-' + fmt(discount)"></span>
                        </div>
                        <div class="row">
                        ...
                        <div class="row total">
                            <span>Total</span>
                            <span class="amt" x-text="fmt(total)"></span>
                        </div>
                        ...
                        <a href="<?= app_url('/checkout') ?>" class="btn btn-gold btn-lg btn-block">
                            Proceed to Checkout
    ...
    <div x-show="items.length > 0" class="sticky-bar lg-hide">
        <div>
            ...
            <span style="font-family: var(--font-display); font-weight: 520; font-size: 22px; color: var(--gold-deep);" x-text="fmt(total)"></span>
        </div>
        <a href="<?= app_url('/checkout') ?>" class="btn btn-gold btn-lg">Checkout Now</a>
    </div>
</div>

<script>
function cartPage() {
    var cur = (window.MS_CURRENCY || { selected: 'NGN', base: 'NGN', rates: {} });
    function fmt(n) {
        n = Number(n || 0);
        var code = cur.selected || cur.base;
        var sym = { NGN: '\u20A6', USD: '$', GBP: '\u00A3', EUR: '\u20AC', GHS: 'GH\u20B5', ZAR: 'R', KES: 'KSh', XAF: 'FCFA', GMD: 'D' };
        var frac = (code === 'NGN' || code === 'XAF') ? 0 : 2;
        var rate = code === cur.base ? 1 : (cur.rates[code] || 1);
        var converted = n * rate;
        var s = converted.toFixed(frac);
        var parts = s.split('.');
        parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ',');
        return (sym[code] || code + ' ') + parts.join('.');
    }
    return {
