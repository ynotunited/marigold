...
<!-- ============================================================
     ICONIC PRODUCTS — pulled from DB (is_featured = true)
     ============================================================ -->
<section class="section" style="padding-top:clamp(56px, 7vw, 96px)">
    <div class="container">
        ...
        <div class="iconic-grid">
            ...
            $iconic_products = [];
            try {
                $allProducts = \App\Core\Catalogue::all();
                $iconic_products = array_filter($allProducts['products'] ?? [], function ($p) {
                    return $p['is_featured'] ?? false;
                });
                $iconic_products = array_slice($iconic_products, 0, 4);
            } catch (\Throwable $e) {}
            foreach ($iconic_products as $i => $prod): ?>
                <a class="iconic-card reveal<?= $i % 4 ? ' d' . $i % 4 : '' ?>" href="<?= app_url('/product/' . htmlspecialchars($prod['id'])) ?>">
                    <div class="ic-img">
                        <span class="ic-idx">0<?= $i + 1 ?></span>
                        <img src="<?= htmlspecialchars($prod['img']) ?>" alt="<?= htmlspecialchars($prod['name']) ?>" loading="lazy">
                    </div>
                    <h3><?= htmlspecialchars($prod['name']) ?></h3>
                    <div class="ic-swatches">
                        <small><?= htmlspecialchars($prod['brand'] ?: $prod['cat']) ?></small>
                    </div>
                </a>
            <?php endforeach; ?>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var items = PRODUCTS.filter(function (p) { return p.is_new; });
