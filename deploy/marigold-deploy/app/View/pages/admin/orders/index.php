...
<div class="bg-[#111] border border-[var(--border)] rounded-[14px] p-4 mb-6 flex items-center gap-3 overflow-x-auto">
    <div class="relative flex-grow min-w-[200px] max-w-sm shrink-0"><i data-lucide="search" class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--text-muted)]"></i><input type="text" placeholder="Order #, customer, email…" class="input-field w-full pl-10 h-9 text-sm bg-[var(--surface)]"></div>
    <input type="date" class="input-field h-9 text-sm bg-[var(--surface)] min-w-[140px] shrink-0">
    <select class="input-field h-9 text-sm bg-[var(--surface)] pr-8 min-w-[140px] shrink-0"><option value="">Payment Status</option><option>Paid</option><option>Pending</option><option>Refunded</option></select>
    <select class="input-field h-9 text-sm bg-[var(--surface)] pr-8 min-w-[150px] shrink-0"><option value="">Delivery Status</option><option>Pending</option><option>Processing</option><option>Shipped</option><option>Completed</option><option>Cancelled</option></select>
</div>
        ...
        <table class="w-full text-left whitespace-nowrap">
            ...
            <tbody class="divide-y divide-[var(--border)]">
                ...
                <tr class="hover:bg-[var(--surface)]/40 transition-colors group cursor-pointer" onclick="window.location='/admin/orders/<?= $o['id'] ?>'">
                    ...
                    <td class="px-5 py-4">
                        <div><p class="text-sm font-medium"><?= htmlspecialchars($o['customer']) ?> <span class="ml-1 inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wide border <?= $o['is_guest'] ? 'bg-gray-500/10 text-gray-400 border-gray-500/20' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' ?>"><?= $o['is_guest'] ? 'Guest' : 'Account' ?></span></p><p class="text-xs text-[var(--text-muted)]"><?= htmlspecialchars($o['email']) ?></p></div>
                    </td>
                    ...
                    <td class="px-5 py-4 text-right font-bold text-sm"><?= money_format($o['total'], 'NGN') ?></td>
                    <td class="px-5 py-4" onclick="event.stopPropagation()">
                        <div class="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <a href="<?= app_url('/admin/orders/') ?><?= $o['id'] ?>" class="w-8 h-8 rounded-[6px] bg-[var(--surface)] border border-[var(--border)] flex items-center justify-center text-[var(--text-secondary)] hover:text-[var(--gold)] hover:border-[var(--gold)]/50 transition-colors"><i data-lucide="eye" class="w-3.5 h-3.5"></i></a>
