        ...
        <div class="bg-[#111] border border-[var(--border)] rounded-[16px] overflow-hidden">
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-[var(--border)]">
                    ...
                    <tr>
                        ...
                        <td class="px-5 py-4 text-right text-sm"><?= money_format($item['price'], 'NGN') ?></td>
                        <td class="px-5 py-4 text-right font-bold text-sm text-[var(--gold)]"><?= money_format($item['total'], 'NGN') ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
            <div class="p-5 bg-[var(--surface)] border-t border-[var(--border)]">
                <div class="space-y-2 max-w-xs ml-auto text-sm">
                    <div class="flex justify-between text-[var(--text-secondary)]"><span>Subtotal</span><span><?= money_format($order['subtotal'], 'NGN') ?></span></div>
                    <div class="flex justify-between text-[var(--text-secondary)]"><span>VAT (<?= round(($order['tax'] > 0 ? $order['tax'] / max($order['subtotal'], 1) : 0) * 100, 2) ?>%)</span><span><?= money_format($order['tax'], 'NGN') ?></span></div>
                    <div class="flex justify-between text-[var(--text-secondary)]"><span>Shipping</span><span><?= money_format($order['shipping'], 'NGN') ?></span></div>
                    <div class="flex justify-between font-bold text-base pt-2 border-t border-[var(--border)]"><span>Total</span><span class="text-[var(--gold)]"><?= money_format($order['total'], 'NGN') ?></span></div>
                </div>
            </div>
        </div>
    ...
    <div class="xl:col-span-1 space-y-6">
        <div class="bg-[#111] border border-[var(--border)] rounded-[16px] p-6">
            ...
            <div class="flex items-center gap-3 mb-4">
                ...
                <span class="ml-auto inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wide border <?= !empty($order['customer']['is_guest']) ? 'bg-gray-500/10 text-gray-400 border-gray-500/20' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' ?>"><?= !empty($order['customer']['is_guest']) ? 'Guest' : 'Account' ?></span>
            </div>
