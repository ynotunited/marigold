...
<div class="product-card group flex flex-col h-full">
    ...
    <div class="p-5 flex flex-col grow">
        ...
        <div class="mt-auto flex items-center gap-3">
            <?php
            $productCurrency = \App\Core\Session::get('currency') ?? 'NGN';
            ?>
            <?php if ($isOnSale): ?>
                <span class="text-[var(--gold)] font-bold text-xl"><?= \App\Core\Money::format($product['sale_price'], $productCurrency) ?></span>
                <span class="text-[var(--text-secondary)] line-through text-sm"><?= \App\Core\Money::format($product['price'], $productCurrency) ?></span>
            <?php else: ?>
                <span class="text-[var(--gold)] font-bold text-xl"><?= \App\Core\Money::format($product['price'], $productCurrency) ?></span>
            <?php endif; ?>
        </div>
    </div>
</div>
